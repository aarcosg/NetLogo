...
[
  ...
  ; Variables para comunicación en radio usadas por el PSO:
  radio-mejor-val    ; mejor valor encontrado por el PS en el radio
  radio-mejor-x      ; coordenada x del mejor valor encontrado por el PS en el radio
  radio-mejor-y      ; coordenada y del mejor valor encontrado por el PS en el radio
]
...
to go
    ...
    ;; Determinar las mejores partículas en un radio dado.
    ;; Para cada partícula, seleccionar la mejor partícula encontrada en un radio dado
    ;; y guardarla en las propiedades de la partícula.
    ask particulas [
      let mejor-particula-radio max-one-of particulas in-radius radio [personal-mejor-val]
      set radio-mejor-val [personal-mejor-val] of mejor-particula-radio
      set radio-mejor-x [personal-mejor-x] of mejor-particula-radio
      set radio-mejor-y [personal-mejor-y] of mejor-particula-radio
    ]
  ...
  [
    ...
    ; Cambia la velocidad para ser atraído por la partícula "mejor en radio" que ha sido encontrado
    facexy radio-mejor-x radio-mejor-y
    set dist distancexy radio-mejor-x radio-mejor-y
    set v  +v v (*v ((1 - inercia-particula) * atraccion-al-radio-mejor * (random-float 1.0) * dist) (list dx dy))
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot max [radio-mejor-val] of particulas"
...
488
Mejor aceleracion
(25 + (max [radio-mejor-x] of particulas)) / 5000
3
1
11
...
488
Mejor decelaracion
(25 + max [radio-mejor-y] of particulas) / 500
3
1
11
...
## EJERCICIO 3 - PSO RADIO

Implementa un PSO en el que la comunicación entre partículas venga determinada por la posición que ocupan, es decir, que cada partícula solo puede saber los valores alcanzados por partículas que están dentro de un determinado radio.

## SOLUCIÓN

Para que la comunicación de las partículas se efectúe con otras partículas dentro de un determinado radio, en lugar de crear variables globales con los mejores valores encontrados, se han añadido a las partículas 3 propiedades:

- radio-mejor-val: mejor valor encontrado por el PS en en el radio.
- radio-mejor-x: coordenada x del mejor valor encontrado por el PS en el radio.
- radio-mejor-y: coordenada y del mejor valor encontrado por el PS en el radio.

Primero se calculan los valores personales de cada partícula. Y después, para cada partícula, se busca la mejor partícula que haya en un radio determinado. Hacer esto es muy fácil gracias a las características de Netlogo:

	let mejor-particula-radio max-one-of particulas in-radius radio [personal-mejor-val]

El radio de comunicación se indica con el slider RADIO de la interfaz gráfica.
