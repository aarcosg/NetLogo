...
to setup
  ...
  [
    ...
    let val f ((xcor + 100) / 200) ((ycor + 100) / 200)
...
to go
  ask turtles [
    ...
    ;calcular valor asociado a la partícula
    let val f ((xcor + 100) / 200) ((ycor + 100) / 200)
...
;; Función aleatoria sin ningún significado
to-report f [x y]
  report one-of [-1 1] * x - one-of [-1 1] * y
end
...
## EJERCICIO 1 - PSO SIN DISCRETIZACIÓN

El algoritmo PSO visto en clase usa una discretización de la función que se está optimizando por medio de los patches. Haz los cambios necesarios para que se pueda usar con funciones no discretizadas sin considerar los patches (esto es, si tenemos una función f(x,y), cada partícula no mira el valor que hay en el patch que tiene debajo, sino que directamente evalúa el valor de f en su posición).

## SOLUCIÓN

Para la resolución de este ejercicio se ha tomado como base el modelo visto en clase en el que f(x,y) depende de los patches. Se han eliminado todas las referencias a los patches y ahora cada partícula no mira el valor del patch en el que se encuentra, sino que evalúa el valor de la función f(x,y) en su posición -> f(xcor,ycor).

Esto permite que las dos coordenadas, x e y, se puedan utilizar como parámetros de la función de fitness que se desea optimizar en los distintos problemas en los que se puede aplicar este algoritmo.
...
El espacio de búsqueda en este modelo es una función aleatoria sin más significado.
