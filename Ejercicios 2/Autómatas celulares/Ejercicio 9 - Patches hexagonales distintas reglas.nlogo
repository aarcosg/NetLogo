...
to setup
  ...
  ask patch 0 0
    [
      ask cells-here[
        set color white
        if num-celdas-iniciales > 1 [
          ask n-of (num-celdas-iniciales - 1) hex-neighbors [
            set color white
          ]
        ]
      ]
...
to setup-juego-vida
  ...
  setup-grid
  ask patches
    [ if random-float 100.0 < densidad-inicial
      [      ask cells-here
        [ set color white ]
      ]
    ]
...
to go
  ;; ejecutar regla del automáta celular
  if regla = "Regla 1"
  [regla-1]
  if regla = "Regla 2"
  [regla-2]
...
to regla-juego-vida
  ;; contar celdas blancas (vivas)
  ask cells[
    set n count hex-neighbors with [color = white]
  ]
  ;; ejecutar regla del juego de la vida
    ask cells [
      ifelse n = 3
      ;; nace célula -> poner celda en blanco
      [ set color white]
      [ if n != 2
        ;; muere célula -> poner celda en gris
        [ set color gray - 3 ]
      ]
    ]
end
...
to regla-2
  ...
  ask cells[
    if n = 1
    [
      ask hex-neighbors [set color white]
      set color gray - 3
    ]
  ]
end
...
regla
regla
"Regla 1" "Regla 2" "Regla 3" "Copo nieve"
1
...
Haciendo uso del ejemplo que viene en la biblioteca de NetLogo de patches hexagonales, define autómatas celulares 2D que funcionen sobre este tipo de malla.

### CÓMO FUNCIONA

1. Seleccionar el número de celdas iniciales.
2. Seleccionar la regla del autómata celular que se desea aplicar.
3. Pulsar el botón SETUP para inicializar el modelo.
4. Pulsar el botón GO o STEP para comenzar la simulación.

La simulación del Juego de la Vida con patches hexagonales funciona de forma análoga:

1. Seleccionar la densidad de células vivas (blancas).
2. Pulsar el botón SETUP.
3. Pulsar el botón GO.

### SIMULACIONES INTERESANTES
Las reglas 1 y 3 producen efectos visuales muy llamativos.
