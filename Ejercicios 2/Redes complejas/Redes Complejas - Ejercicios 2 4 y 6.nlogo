...
distancia-desde-otros-vertices   ;; lista de distancias desde el vertice a otros
]

globals[
coeficiente-clustering-total
diametro
]
...
to go
  ...
  [make-node find-partner]        ;; find partner & use it as attachment
                                  ;; point for new node
  if add-edges?                   ;; Ejercicio 2. Añadir aristas para comparar
                                  ;; la evolución de la distribución de grados en el grafo
  [make-edge]

  calcula-coeficiente-clustering
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Ejercicio 2. Crear aristas entre dos nodos aleatorios ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
to make-edge
  let contador-aristas count links
  let contador-vertices count turtles
  let num-maximo-aristas  (contador-vertices * (contador-vertices - 1)) / 2
  let arista-creada? false
  ifelse (contador-aristas + 1) <= num-maximo-aristas
  [
    while [not arista-creada?][
      ask one-of turtles[
        let v one-of other turtles
        if (link-with v = nobody)
        [
          create-link-with v
          set arista-creada? true
        ]
      ]
    ]
  ]
  [user-message( "Se ha alcanzado el número máximo de aristas. Pulsa DETENER para finalizar" )]
end
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Ejercicio 4. Cálculo de coeficiente clustering ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;


to calcula-coeficiente-clustering

  ifelse all? turtles [count link-neighbors <= 1]
  [
    ;; Si todos los nodos tienen uno o menos vecinos conectados,
    ;; el coeficiente de clustering es cero.
    set coeficiente-clustering-total 0
  ]
  [
    ;; En el caso de que haya algún nodo con más de una conexión con otro nodo,
    ;; iterar sobre ellos
    let total 0
    ...
    ask turtles with [ count link-neighbors > 1]
    [
      let vecinos link-neighbors
      ;; Calcular el coeficiente de clustering del nodo
      set coeficiente-clustering (count links with [ member? end1 vecinos and member? end2 vecinos ] /
                                         ((count vecinos) * (count vecinos - 1) / 2) )
      ;; Sumar el coeficiente de clustering del nodo al total
      set total total + coeficiente-clustering
    ]
    ;; Establecer como coeficiente de clustering total la media de los coeficientes de los nodos
    set coeficiente-clustering-total total / count turtles with [count link-neighbors > 1]
  ]
end

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Ejercicio 6. Cálculo de diámetro del grafo ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;; Función que implementa el algoritmo Floyd Warshall para calcular
;; las distancias más cortas entre pares de nodos del grafo.

to calcula-distancias
  let infinito 99999999999
  ;; reset the distance list
  ask turtles
  [
    set distancia-desde-otros-vertices []
  ]
  ...
  [
    ...
    [
      ...
      [
        ask node1 [
          set distancia-desde-otros-vertices lput 0 distancia-desde-otros-vertices
        ]
      ]
      [
        ...
        [
          ask node1 [
            set distancia-desde-otros-vertices lput 1 distancia-desde-otros-vertices
          ]
        ]
        ;; infinite to everyone else
        [
          ask node1 [
            set distancia-desde-otros-vertices lput infinito distancia-desde-otros-vertices
          ]
        ]
      ]
      set j j + 1
  ...
  [
    ...
    [
      ...
      [
        ...
        set temp ( (item k [distancia-desde-otros-vertices] of turtle i) +
                    (item j [distancia-desde-otros-vertices] of turtle k))
        ;; is the alternate path shorter?
        if temp < (item j [distancia-desde-otros-vertices] of turtle i)
        [
          ask turtle i [
            set distancia-desde-otros-vertices replace-item j distancia-desde-otros-vertices temp
          ]
        ]
        set j j + 1
      ]
...
;; Función que calcula el diámetro del grafo
;; Se debe ejecutar posteriormente a la función calcula-distancias
to calcula-diametro
  ask turtles [
    if max distancia-desde-otros-vertices > diametro
    [set diametro max distancia-desde-otros-vertices ]
  ]
end
...
"" ""
PENS
"nodes" 1.0 1 -16777216 true "" "if not plot? [ stop ]\nif add-nodes?[\nlet max-degree max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\nset-plot-x-range 1 (max-degree + 1)  ;; + 1 to make room for the width of the last bar\nhistogram [count link-neighbors] of turtles\n]"
"edges" 1.0 1 -2674135 true "" "if not plot? [ stop ]\nif add-edges?[\nlet max-degree max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\nset-plot-x-range 1 (max-degree + 1)  ;; + 1 to make room for the width of the last bar\nhistogram [count link-neighbors] of turtles\n]"
...
"" ""
PENS
"clustering" 0.1 1 -16777216 true "set-plot-x-range 0 1.1\nset-histogram-num-bars 10" "if not plot? [ stop ]\n;;let max-cc max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\n;;set-plot-x-range 1 (max-degree + 1)  ;; + 1 to make room for the width of the last bar\nhistogram [precision coeficiente-clustering 1] of turtles"
...
Haciendo uso del modelo "Preferential Attachment" que viene en la biblioteca de ejemplos de NetLogo, hacer un modelo que muestre en tiempo real la evolución de la distribución de grados del grafo que forma. A continuación, añadid un procedimiento en el que se añadan aristas aleatoriamente entre nodos del grafo y medir cómo evoluciona la distribución de grados.

### CÓMO FUNCIONA
1. Encender el interruptor ADD-NODES? y apagar el interruptor ADD-EDGES?.
2. Pulsar el botón SETUP para inicializar el grafo.
3. Pulsar el botón GO para que la simulación se ejecute durante un tiempo.
4. Pulsar de nuevo el botón GO para que la ejecución pare.
5. Apagar el interruptor ADD-NODES? y encender el interruptor ADD-EDGES?.
6. Volver a pulsar el botón GO y dejar que se ejecute durante un tiempo.

En esta simulación se puede observar como al principio cuando no se añaden aristas, la gráfica de distribución de grados es similar a la gráfica de redes reales vista en el tema.

Sin embargo, cuando se empiezan a añadir aristas, el gráfico tiende a una distribución normal, como la que se produce en las redes aleatorias.

### VISUALIZACIÓN

En las gráficas de la interfaz de usuario no se borran los datos cuando se pasa al modo de añadir aristas para que se pueda comprobar fácilmente cómo evoluciona la distribución de grados.

En negro se representa el modo añadir nodos, y en rojo el modo añadir aristas.

## EJERCICIO 4

Define un procedimiento en NetLogo que permita calcular el coeficiente de clustering de los nodos de un grafo. A continuación aplícalo sobre un grafo cualquiera para calcular su distribución de clustering.

### CÓMO FUNCIONA
1. Encender los interruptores de añadir nodos y aristas, ya que si no se añaden aristas el coeficiente de clustering será cero.
2. Pulsar el botón SETUP para inicializar la red.
3. Pulsar el botón GO para que se ejecute la simulación.

Se recomienda activar y desactivar los interruptores de añadir nodos y aristas para ver como evoluciona la red en ambos casos.

### VISUALIACIÓN
En el monitor COEFICIENTE CLUSTERING RED se muestra el coeficiente de clustering de la red mientras se ejecuta la simulación.

En la gráfica DISTRIBUCIÓN DE CLUSTERING se puede ver la evolución de la distribución del coeficientes de clustering.

## EJERCICIO 6

Define un procedimiento para poder calcular en NetLogo el diámetro de un grafo.

### CÓMO FUNCIONA
1. Pulsar el botón CALCULA DIÁMETRO cuando haya alguna red dibujada.

### VISUALIZACIÓN
El diámetro de la red aparece en el monitor DIÁMETRO DE LA RED.
