...
to crea-red
  ...
  set inicio one-of nodos
  ask inicio [set color green set size 2]
  set fin one-of nodos with [distance inicio > max-pxcor]
...
to-report heuristica [#fin]
  ...
  if seleccionar-heuristica = "Manhattan"[
    let diferencia-x abs(xcor - [xcor] of #fin)
    let diferencia-y abs(ycor - [ycor] of #fin)
...
to-report A* [#Inicio #Fin #Visible]
  ...
  ask buscadores [die]
  output-print (word "Heurística: " seleccionar-heuristica " = " precision (Tiempo_fin - tiempo_inicio) 3 )
  report res
end
...
Modelo que aplica distintas heurísticas a un grafo geométrico utilizando el algoritmo A*.

## HOW IT WORKS

Se genera un grafo geométrico utilizando el número de nodos y el radio establecido en la interfaz. A este grafo, se le añaden dos puntos, inicio y fin. El algoritmo A* se encargará de encontrar el camino óptimo entre ambos puntos utilizando la heurística seleccionada. En la ventana de salida, se podrá observar y comparar el tiempo de ejecución.

## HOW TO USE IT

Seleccionar el radio que emplearán los nodos para conectarse unos con otros, el número de nodos y la heurística que utilizará el algoritmo A*.

Una vez realizado esto, pulsar en el botón "Setup" para que se cree el grafo y seguidamente en el botón "Test". Cuando finalice, se puede volver a ejecutar el algoritmo pulsando de nuevo el botón "Test", por lo tanto se recomienda ir seleccionando distintas heurísticas para comprobar el comportamiento de cada una.

## THINGS TO NOTICE

En los tests realizados, la heurística más rápida en encontrar el camino ha sido "Diagonal Tie Breaker".

## CREDITS AND REFERENCES

1. http://theory.stanford.edu/~amitp/GameProgramming/
2. Modelo de James P. Steiner: https://github.com/fsancho/IA/blob/master/Busquedas%20Informadas/a-star_2009.nlogo
3. https://github.com/fsancho/IA/blob/master/Busquedas%20Informadas/A-estrella%20Tortugas.nlogo
@#$#@#$#@
default
true
0
